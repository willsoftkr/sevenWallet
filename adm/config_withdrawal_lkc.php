...
$sql = " select count(*) as cnt from pinna_btc_trans A inner join g5_member M on A.mb_id = M.mb_id WHERE 1=1 and type = 8 ";
$sql .= $sql_condition;
...
$sql = "select * from pinna_btc_trans A inner join g5_member M on A.mb_id = M.mb_id WHERE 1=1 and type = 8";
$sql .= $sql_condition;
...
<script>
	$(function(){
		$('.regTb [name=status]').on('change',function(e){
			$.post( "/adm/config_withdrawal_lkc.u.php", {
				uid : $(this).attr('uid'),
				status : $(this).val()
			}, function(data) {
				alert('상태 변경 성공.')
	...
	<table cellspacing="0" cellpadding="0" border="0" class="regTb">
		...
		<?for ($i=0; $row=sql_fetch_array($list); $i++) {?>
			<tr>
				...
				<?if($row[lkc_coin_num]<0){?>
				<td style="color:red"><?=$row[lkc_coin_num]?></td>
				<?}else{?>{
				<td><?=$row[lkc_coin_num]?></td>
				<?}?>
