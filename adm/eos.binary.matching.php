...
//$sub_menu = "600600";
include_once('./_common.php'); //실서버용 경로
...
function save_benefit($day, $mbid, $mbname, $recom, $allowance_name, $sales_day, $habu_day_sales, $benefit, $rec_adm,$rec,$mblevel ){
	//수당을 비트로 환산 한다.
	$benefit_bit = round($benefit,8);
	//회원 잔고에 더해 준다.
				...
				if($cond[$i]['per']!=''){ // 수당 퍼센트가 비어있으면 패스~
								...
								if( ($bm==0) && ($temp_cond_level1==0)  ){
									...
									if( ($pay>0) && ($cond[$i]['base_source']=='Cycle') ) {
										...
										save_benefit($to_date, $mbid, $mbname, $recom, $cond[$i]['allowance_name'], $recommend['sales_day'], $recommend['habu_day_sales'], ($benefit*$cond[$i]['per']/100) , $cond[$i]['allowance_name'].' '.$rec_adm, $rec, $mblevel);
									}
								}
