...
<div class="tbl_head02 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col" id="th_odrall" >입금전환금액(btc)</th>
		<th scope="col" id="th_odrcnt" >전환된입금액(USD)</th>
		<th scope="col" id="th_odrcnt" >전환비율(USD)</th>
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td class="td_payby">
			...
			<!-- ##start##  ##### -->
			<? if ($row['od_receipt_cash'] > 0 && $row['od_receipt_price'] > 0) { ?>
			현금+<?php echo $s_receipt_way; ?>
			<? } else if ($row['od_receipt_cash'] > 0 && $row['od_receipt_price'] == 0) { ?>
			현금
			<? } else { ?>
		...
		<td rowspan="2" class="td_numsum"><?= number_format($row['od_cart_price'],8)?></td>
