...
function make_habu($gubun){
	...
	if ($order_proc==1){
		habu_sales_calc($gubun,'admin',0,0); 
	}
}
...
function habu_sales_calc($gubun, $recom, $deep){
    ...
    for ($j=0; $rrr=sql_fetch_array($res); $j++) { 
			...
			if( ($noo>0) && ($nooyn==0)) {
				$nooyn=1;
				sql_query("insert ".$gubun."noo SET noo=".$noo." ,mb_id='".$recom."'");	
				//echo 'noo:'.$recom.'  '.$deep.'<br>';
			}
			
			if(($mon>0)&& ($monyn==0) ) {
				$monyn=1;
				sql_query("insert ".$gubun."thirty SET thirty=".$mon." ,mb_id='".$recom."'");
				//echo 'thirty:'.$recom.'  '.$deep.'-'.$olddeep.'<br>';
			}
			
			if(($today>0)&& ($todayyn==0)) {
				$todayyn=1;
				sql_query("insert ".$gubun."today SET today=".$today." ,mb_id='".$recom."'");
			}
		...
		if ($count){
			...
			for ($i=0; $row=sql_fetch_array($result); $i++) {
				...
				if ($row[b_recomm]){
					...
					$sql  = "select (mb_my_sales+habu_day_sales) as tpv from g5_member where mb_id ='".$row[b_recomm]."'";
					$row6 = sql_fetch($sql);
				...
				//바이너리 오른쪽 오늘 매출
				if ($row[b_recomm2]){
					$sql  = "select (mb_my_sales+habu_day_sales) as tpv from g5_member where mb_id ='".$row[b_recomm2]."'";
