					...
					if( $hap1<=$hap2 ){ //$hap1이 소실적이라면
						if($hap1>=($cond[$i]['sales_reset']) ){ //소실적이 극점?
							...
							if( $cond[$i]['max_reset1'] =='대.소실적 모두이월'){
								...
								if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate);
								...
								$note_adm='극점초과(대실적만 이월) (2) 소실적:'.$hap1.	'('.$id1.') / 대실적:'.$hap2.	'('.$id2.') 이월금:'.($hap2-$cond[$i]['sales_reset']);
								echo $note='Binary Cycle Bonus for '.$maxcycle.' cycles as a '.$deslv.' member';
								$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
								if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate)
								iwol_process($to_date, $mbid, $id2, $mbname, 2,$hap2-$cond[$i]['sales_reset'] , $note_adm); 
								...
								$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
								if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate);
								iwol_process($to_date,$mbid,  $id1, $mbname, 3,$hap1-$cond[$i]['sales_reset'] , $note_adm); 
							}
						} else{ //극점이 아니고
							if($hap1>=($cond[$i]['cycle'])){ // 1Cycle보다 크다
								...
								if($cond[$i]['max_reset2']=='대.소실적 모두이월'){
									$note_adm=$cnt.'Cycle달성(대.소실적 모두이월) (4) 소실적:'.$hap1.	'('.$id1.') / 대실적:'.$hap2.	'('.$id2.')';
									echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
									$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm, $note, $exchange_rate);
									iwol_process($to_date, $mbid, $id1, $mbname, 4, $hap1-($cnt*($cond[$i]['cycle']) )  , $note_adm); 
									iwol_process($to_date, $mbid, $id2, $mbname, 4, $hap2-($cnt*($cond[$i]['cycle']) )  , $note_adm); 
								} else if($cond[$i]['max_reset2']=='대실적만 이월'){
									$note_adm=$cnt.'Cycle달성(대실적만 이월) (5) 소실적:'.$hap1.	'('.$id1.') / 대실적:'.$hap2.	'('.$id2.') 이월금:'.($hap2-$cond[$i]['sales_reset']);
									echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
									$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) 
										save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm,$note, $exchange_rate);
										iwol_process($to_date, $mbid,  $id2, $mbname, 5, $hap2-$cond[$i]['sales_reset'] , $note_adm); 
									}else if($cond[$i]['max_reset2']=='소실적만 이월'){
										$note_adm=$cnt.'Cycle달성(소실적만 이월)(6) 소실적:'.$hap1.	'('.$id1.') / 대실적:'.$hap2.	'('.$id2.') 이월금:'.($hap1-$cond[$i]['sales_reset']);
										echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
										$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) 
										save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm,$note, $exchange_rate);
										iwol_process($to_date,$mbid,  $id1, $mbname,6, $hap1-$cond[$i]['sales_reset'] , $note_adm); 
									}
								}else{ // 1Cycle보다 작다
									$note_adm='*Cycle미달 (7) 소실적:'.$hap1.	'('.$id1.') / 대실적:'.$hap2.	'('.$id2.')';
									minus_iwol($id1,$day);
								//iwol_process($to_date, $mbid, $id1, $mbname, 'Cycle미달', $hap1  , $note); 
								//iwol_process($to_date, $mbid, $id2, $mbname, 'Cycle미달', $hap2  , $note); 
								}
							}
						} 
						else{ //$hap2가 소실적이라면
							if($hap2>=($cond[$i]['sales_reset']) ){ //소실적이 극점?
								...
								$note_adm='극점초과(대실적만 이월) (9) 소실적:'.$hap2.	'('.$id2.') / 대실적:'.$hap1.	'('.$id1.') 이월금:'.($hap1-$cond[$i]['sales_reset']);
								echo $note='Binary Cycle Bonus for '.$maxcycle.' cycles as a '.$deslv.' member';
								$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
								if($today_sales>0) 
									save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales,  $cond[$i]['per'],$cond[$i]['allowance_name'].' '.$note_adm, $note, $exchange_rate);
								iwol_process($to_date, $mbid, $id1, $mbname, 9,$hap1-$cond[$i]['sales_reset'] , $note_adm); 
							...
							if($hap2>=($cond[$i]['cycle'])){ // 1Cycle보다 크다
								...
								if($cond[$i]['max_reset2']=='대.소실적 모두이월'){
									$note_adm=$cnt.'Cycle달성(대.소실적 모두이월) (11) 소실적:'.$hap2.	'('.$id2.') / 대실적:'.$hap1.	'('.$id1.')';
									echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
									$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales,  $cond[$i]['per'],$cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate);
									iwol_process($to_date, $mbid, $id1, $mbname, 11, $hap1-($cnt*($cond[$i]['cycle']) )  , $note_adm); 
									iwol_process($to_date, $mbid, $id2, $mbname,11, $hap2-($cnt*($cond[$i]['cycle']) )  , $note_adm); 
								}else if($cond[$i]['max_reset2']=='대실적만 이월'){
									$note_adm = $cnt.'Cycle달성(대실적만 이월) (12) 소실적:'.$hap2.	'('.$id2.') / 대실적:'.$hap1.	'('.$id1.') 이월금:'.($hap1-$cond[$i]['sales_reset']);
									echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
									$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) 
										save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales,  $cond[$i]['per'],$cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate);
									iwol_process($to_date, $mbid, $id1, $mbname,12,$hap1-$cond[$i]['sales_reset'] , $note_adm); 
								}else if($cond[$i]['max_reset2']=='소실적만 이월'){
									$note_adm = $cnt.'Cycle달성(소실적만 이월)(13) 소실적:'.$hap2.	'('.$id2.') / 대실적:'.$hap1.	'('.$id1.') 이월금:'.($hap2-$cond[$i]['sales_reset']);
									echo $note = 'Binary Cycle Bonus for '.$cnt.' cycles as a '.$deslv.' member';
									$no_benefit=1;$binary_firstname=$mbname;$binary_firstid=$mbid;
									if($today_sales>0) save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], '', 0, $today_sales, $cond[$i]['per'], $cond[$i]['allowance_name'].' '.$note_adm, $note,$exchange_rate);
									iwol_process($to_date, $mbid, $id2, $mbname, 13,$hap2-$cond[$i]['sales_reset'] , $note_adm); 
								}
							}else{ // 1Cycle보다 작다
...
function btoday_select($mbid){
	$res= sql_fetch("select todayy from btoday where mb_id='".$mbid."'");
	if($res['todayy']=='')
	{
		$hap=0;
	}else{
...
function habu_iwol($mbid,$day,$cycle){
	...
	$hap1=(btoday_select($mbid)+today_sales($mbid,$day));  //자기매출과 하부매출을 합하여 
	$res2= sql_fetch("select sum(pv)as hap from iwol where mb_id='".$mbid."'");
	$hap2=$res2['hap'];
	echo '#########'.$hap1.'+'.$hap2.'<'.$cycle.' && '.$hap1.'>0';
	if(  (($hap1+$hap2)<$cycle) && ($hap1>0) ){ //만약 cycle보다 작아 이월 및 수당 안하지만 오늘매출이 있을경우 오늘매출을 이월한다
		plus_iwol($mbid,$day);
	}
	return ($hap1+$hap2);    	
} 
function my_bchild($mb_id,$day,$cycle){
	...
	if(($hap1>=$cycle) && ($hap2>=$cycle)){ // cycle보다 같거나 크면 -이월한다
		minus_iwol($id1,$day); 
		minus_iwol($id2,$day); 
	}
