		...
		if ($count){
			...
			for ($i=0; $row=sql_fetch_array($result); $i++) {
				...
				if ($gubun=="B"){
					...
					if ($row['mb_lr']==1 && $row['mb_brecommend_type']=="R"){
						$sql = "update g5_member set mb_brecommend_type='L' where mb_id='".$row['c_id']."'";
						sql_query($sql);
					}

					if ($row['mb_lr']==2 && $row['mb_brecommend_type']=="L"){
						$sql = "update g5_member set mb_brecommend_type='R' where mb_id='".$row['c_id']."'";
						sql_query($sql);
					}
				...
				if ($gubun=="B"){
					...
					if ($row1['cnt']==2 && (!$row['b_recomm'] || !$row['b_recomm2'])){
						...
						//$sql = "select * from g5_member m join ".$class_name." c on m.mb_id=c.mb_id where c.mb_id='{$member[mb_id]}' and length(c.c_class)=".($clen+2)." and c.c_class like '".$row[c_class]."%' order by mb_lr ";
						$sql = "select * from g5_member where mb_brecommend='".$row[c_id]."' order by mb_lr asc";
						$tres = sql_query($sql);
						for ($j=0; $trow=sql_fetch_array($tres); $j++) {
							if ($j==0){
								$sql = "update g5_member set mb_lr=1,mb_brecommend_type='L' where mb_id='".$trow['mb_id']."'";
								$row['b_recomm'] = $trow['mb_id'];
							}else{
								$sql = "update g5_member set mb_lr=2,mb_brecommend_type='R' where mb_id='".$trow['mb_id']."'";
								$row['b_recomm2'] = $trow['mb_id'];
							}
							sql_query($sql);
						}
					} 
...
?>
						<li>[<?=(strlen($row[c_class])/2)-$mdepth?>-<?=($row[c_child])?>-<?=($row[b_child]-1)?>]|<?=get_member_label($row[mb_level])?>|<?=$row[c_id]?>|<?=$row[c_name]?>|<?=number_format($row3[tpv]/1000)?>|<?=number_format($row5[tpv]/1000)?>|<?=$row[mb_level]?>|<?=number_format($row6[tpv]/$order_split)?> - <?=number_format($row7[tpv]/$order_split)?>|<?=$row[pool_level]?>
<?
