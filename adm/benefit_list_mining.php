...
$benefit = "SELECT allowance_name FROM pinna_soodang_mining_pay WHERE (1) GROUP BY allowance_name ";
...
<div class="local_ov01 local_ov">
	...
	<input type="submit" name="act_button" value="  수당 일괄 실행  "  class="frm_input" onclick="go_calc(7);">
    <input type="submit" name="act_button" value="  스폰서관련 수당계산 "  class="frm_input" onclick="go_calc(0);">
	<input type="submit" name="act_button" value="  바이너리보너스 수당계산  "  class="frm_input" onclick="go_calc(1);">
	<input type="submit" name="act_button" value="  바이너리매칭수당계산  "  class="frm_input" onclick="go_calc(2);">
	<input type="submit" name="act_button" value="  승급실시  "  class="frm_input" onclick="go_calc(3);">
	<input type="submit" name="act_button" value="  수당배치  "  class="frm_input" onclick="go_calc(4);">
	<input type="submit" name="act_button" value="  OneStar승급  "  class="frm_input" onclick="go_calc(5);">
	<input type="submit" name="act_button" value="  마이닝 수당  "  class="frm_input" onclick="go_calc(6);">
...
<select name="sfl" id="sfl">
    ...
    <option value="day"<?php echo get_selected($_GET['sfl'], "day"); ?>>날짜</option>>
</select>
...
/*
	if( (document.getElementById("save_noo_mon").checked==false) && (document.getElementById("save_benefit").checked==false) )
	{
		alert('매출계산 혹은 수당계산을 체크하시고 사용하세요!!');
		return;
	}
*/		
/*
if(document.getElementById("sales_yn_chk").checked==true){
	if(!confirm("기존에 집계한 오늘의 매출자료는 모두 지워지고 새롭게 집계합니다. 진행하시겠습니까?")){
		return ;
	}
}
*/
if(document.getElementById("benefit_yn_chk").checked==true){
	if(!confirm("기존에 수당자료를 모두 지우고 새롭게 계산합니다. 진행하시겠습니까?")){
		return ;
	}
}

/*
	if(document.getElementById("save_noo_mon").checked==true)
	{
		str='save_noo_mon=1';
	}
	if(document.getElementById("save_benefit").checked==true)
	{
		if(str!='') {str=str+='&'; }
		str=str+'save_benefit=1';
	}
*/
...
/*
	if(document.getElementById("noo_start_day").value==''){

		alert('누적매출을 시작할 날짜를 입력하세요');
		return;

			
	}else{
		str=str+'&noo_start_day='+document.getElementById("noo_start_day").value;
	}
*/
...
*/
	...
	switch(n){
		case 0: 
			location.href='benefit_immediate.php?'+str;
			break;
		case 1: 
			location.href='benefit_binary.php?'+str;
			break;
		case 2: 
			location.href='benefit_binary_matching.php?'+str;
			break;
		case 3: 
			location.href='benefit_rank.php?'+str;
			break;
		case 4:
			location.href='../new/soodang_update.php';
			break;
		case 5:
			location.href='grade_onestart_up.php';
			break;
		case 6:
			location.href='../new/mining_soodang_calc.php';
			break;
		case 7:
			location.href='total_soodang_run.php?'+str;
			break;
	}
