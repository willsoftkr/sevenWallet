...
if($_POST[status]=='N'){
	$get_row = "Select * from pinna_eos_trans where uid  = ".$_POST[uid];
	$slamdunk = sql_query($get_row);
	$ret = sql_fetch_array($slamdunk);
	$mb_id = $ret['mb_id'];
	$amt = $ret['amt']+0.002;
	$sql1 = "update g5_member set mb_balance = round(mb_balance+".$amt.",8) where mb_id='".$mb_id."'";
	sql_query($sql1);
}

$sql = " update pinna_eos_trans set status = '".$_POST[status]."' ";
$sql .= ", update_dt = now() ";
...
function send_mail($uid, $cf_admin_email_name, $cf_admin_email){
	$i_mem = "Select * from pinna_eos_trans where uid  = ".$uid;
