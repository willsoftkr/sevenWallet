...
$sql = " select count(*) as cnt, sum(amt) as hap from pinna_eos_trans A inner join g5_member M on A.mb_id = M.mb_id WHERE 1=1  and A.type  = 2	 ";
$sql .= $sql_condition;
$sql .= $sql_ord;
...
<div class="adminWrp">
	<span class="total_left">Total : <?=$total_count?></span> <span class="total_right">Total withdrawal EOS : <?echo round($total_hap,8)?> EOS / Total transfer Fee : <?echo $total_count*0.002?> EOS</span>
	<table cellspacing="0" cellpadding="0" border="0" class="regTb">
		...
		<thead>
			...
			<th >메모주소</th>
			<th  style="width:10%;">현재 총 EOS잔고</th>
			<!--<th>현재 EOS 수당 잔고</th>-->
			
			<th style="width:10%;">출금요청금액</th>
			<th style="width:10%;">요청일시</th>
			<th style="width:4%;">승인여부</th>
