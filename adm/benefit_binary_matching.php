...
$prev_day = $row['day'];
if($row['run_status']=='X' && $prev_day!='2018-09-16'){

	$target_day = strtotime("$prev_day + 1 days");

	$to_date =	date("Y-m-d", $target_day);
	$fr_date = 	date("Y-m-d", $target_day);
...
for ($i=0; $row=sql_fetch_array($result); $i++) {   
		...
		$comp=$row['mb_id'];
		$pay=$row['benefit_usd'];
		$benefit=$row['benefit_usd']/$row['benefit_level'];
						...
						if($cond[$i]['per']!=''){ // 수당 퍼센트가 비어있으면 패스~
								...
								if( ($bm==0) && ($temp_cond_level1==0)  ){
									...
									if( ($pay>0) && ($cond[$i]['base_source']=='Cycle') ) {
										...
										$rec_adm=$binary_firstname.'('.$binary_firstid.') 로부터 '.$cond[$i]['allowance_name'].' 발생, 내 직급: '.$mb_level.', 대수: '.($history_cnt).' Cycle 수: '.$benefit;

										$rec=$cond[$i]['allowance_name'].' for '.$history_cnt.' Cycles'.' from '.$binary_firstid.'( level '.$history_cnt.')';
										//echo $mbname.'('.$mbid.'): '.$rec.'<br>';

										save_benefit($to_date,$mbid, $mbname, $recom, $cond[$i]['allowance_name'], $recommend['sales_day'], $recommend['habu_day_sales'],($benefit*$cond[$i]['per']) , $cond[$i]['allowance_name'].' '.$rec_adm, $rec,$exchange_rate);
									}
								}
