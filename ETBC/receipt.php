...
<body>
	...
	<form action ="./receipt.i.php" method="POST">
		...
		<div id="content">
			<p class="line1">My Bitcoin Wallet</p>
			...
			<div class="key">
				...
				<p><a href="javascript:void(0);">Pay with Bitcoin</a></p>
				<p>Total amount to pay : <strong><?=$member['mb_balance']+$total_mining_sudang;?> BTC</strong></p>
				<p>FIJI Wallet : <strong><?=$member['mb_balance'];?> BTC</strong></p>
				<p>Mining Wallet : <strong><?=$total_mining_sudang;?> BTC</strong></p>
				<p>My Bitcoin Wallet</p>
			...
			<div class="qr">
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<?} 	else {?>
							<td><input type="text" id="money" placeholder="ex) 0.022" value="" /></td>
							<? }?>
						</tr>
                        <tr>
							<th>Total Amount</th>
							<td><span id="total">0</span> BTC<input type="hidden" name="amt" /></td>
						</tr>
						<tr>
							<th>OTP Auth Code</th>
							<? if($member['mb_balance'] <0){ ?>
                ...
                <p style="text-align:left; font-size:13px;margin-left:10px;">
                    Remittance Charge : <strong>0.002 BTC</strong> <br>
                    Minimum Amount : <strong>0.02 BTC</strong>
                </p>
			</div>
...
<script>
	$('#money').on('keyup',function(e){
		...
		$('#total').html((Number($(this).val()) + 0.002).toFixed(8));
	});
</script>
