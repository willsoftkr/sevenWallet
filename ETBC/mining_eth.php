...
function get_remind($p_date){
	...
	return max(1000 - $diff->days - 10, 0);
}
...
$package = array('1527096045' => 'package 1', '1527096041' => 'package 2', '1527096037' => 'package 3', '1527096030' => 'package 4', '1526013457' => 'package 5', 'POOL6' => 'package 6', 'POOL7' => 'package 7', 'POOL8' => 'package 8', '1515148167' => 'GPU');
...
<head>
	...
	<link rel="stylesheet" href="css/mining_eth/style.css?v=20181212">
			...
			$.ajax({
				...
				success: function(data){
					...
					$.each(result.list, function( index, obj ) {
						...
						row.find('.balance').text('$ ' + obj.profit_days);
			...
			$.ajax({
				...
				success: function(data){
					...
					$('.cycles .cycle .pay').each( function( index, obj ) {
						...
						$('#clone2 p').eq(0).html('Mining: $ ' + mine_usd.toFixed(2) + '<br>(' + mine + ' ETH)');
						$('#clone2 p').eq(1).html('Fee: $ ' + fee_usd.toFixed(2) + '<br>(' + fee + ' ETH)');
						$('#clone2 p').eq(2).html('Total: $ ' + tot_usd.toFixed(2) + '<br>(' + tot + ' ETH)');
	...
	<div class="main-container">
		<div id="body-wrapper" class="big-container-wrapper">
			...
			<div class="bonus-history-title-container">
				<h2 class="gray " data-i18n="mining.title2" >Ethereum Mining Summary</h2>
			</div>
			<input type="hidden" id="total" value="">
			<div class="bonus-history-container shadow">
				<div class="total-hashpower">
					<div class="hash-total">
						<span class="gray">Total ETH Mined:</span> <span class="blue"><?=round($tot_mine['tot_mine'],8);?> BTC ($ <?=round($tot_mine['tot_mine_usd'],2);?>)</span>
					</div>
				</div>
				<hr>
				<div class="package-container">
					...
					?>
						<li rel="<?echo $row_p[it_id]?>" class="active" remainday="<?echo get_remind($row_p['p_date']); ?>" pool_count="<?=$idx++;?>">
							<div class="package">
								<img src="./images/eth_logo.png" width="40" alt="btc icon"><br>
					...
					<table class="table table-hover table-responsive">
						...
						<thead>
							<tr>
								...
								<th data-i18n="mining.th4" >Daily Earning ETH</th>
								<th data-i18n="mining.th5" >ETH Price</th>
