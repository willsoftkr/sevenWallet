...
sql_query("update g5_write_notice set wr_hit = wr_hit + 1  where wr_id ={$wr_id}");
...
SELECT * FROM (SELECT '{$member['mb_no']}', '{$_GET['no']}') AS tmp
WHERE NOT EXISTS (
	SELECT mb_no FROM read_notice WHERE mb_no = '{$member['mb_no']}'
		and wr_id = '{$_GET['no']}'
) LIMIT 1");

$obj->not_read_cnt = sql_fetch("select count(*) as cnt from g5_write_notice where wr_id not in (select wr_id from read_notice where mb_no = {$member['mb_no']})")['cnt'];
