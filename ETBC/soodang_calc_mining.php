...
$item_price = array(1 => 4500, 13500, 22500, 54000, 112500, 80);
	...
	//$today = '2019-02-15';
	$set_day_coin = "insert pinna_coin_rate set day = '".$today."', btc_rate=".$now_btc_cost[btc_cost].", eth_rate = ".$now_btc_cost[eth_cost].", btc_difficulty =".$btc_difficulty.", eth_difficulty ='".$eth_difficulty."'" ;
	
	sql_query($set_day_coin);
...
function set_mining_soodang_1($one_hash_rev, $onep_mhash, $etherate, $bitrate, $to_date){ //재구매 해서 해시 파워를 더하고 나머지는 수당으로 더하나.
	...
	while($mb_row = sql_fetch_array($mb_rst)){
		...
		$p1_repurchase = round(($pool1 * $one_hash_rev *  $rep_p1 / 100 * $bitrate *3.4 * $give_rate), 4);
		$p2_repurchase = round(($pool2 * $one_hash_rev *  $rep_p2 / 100 * $bitrate *3.4 * $give_rate), 4);
		$p3_repurchase = round(($pool3 * $one_hash_rev *  $rep_p3 / 100 * $bitrate *3.4 * $give_rate), 4);
		$p4_repurchase = round(($pool4 * $one_hash_rev *  $rep_p4 / 100 * $bitrate *3.4 * $give_rate), 4);
		$p5_repurchase = round(($pool5 * $one_hash_rev *  $rep_p5 / 100 * $bitrate *3.4 * $give_rate), 4);
		$p_gpu_repurchase = round(($poolg * $onep_mhash   * $rep_gpu / 100 * $etherate * 8 / 300 * $give_rate),4);
		...
		$save_hash .= ", re_purchase_poolg = ".$p_gpu_repurchase;

		sql_query($save_hash);

		if($p1_profit>=0){   //수당 로그 저장
			$up_repurchase = "update pina_mb_hashpower set pool1_hashp = round((pool1_hashp +  $p1_repurchase), 4) where mb_id='$id'";
			sql_query($up_repurchase);
			echo 'cnt : '.$cnt = $cnt+1;
			save_mining_slog(1, $p1_profit, $p1_repurchase, $id, $to_date, $cnt, $bitrate, $etherate);
		}
		if($p2_profit>=0){
			 $up_repurchase = "update pina_mb_hashpower set pool2_hashp = round((pool2_hashp +  $p2_repurchase), 4) where mb_id='$id'";
			sql_query($up_repurchase);
			$cnt = $cnt+1;
			save_mining_slog(2, $p2_profit, $p2_repurchase, $id, $to_date, $cnt, $bitrate, $etherate);
		}
		if($p3_profit>=0){
			echo $up_repurchase = "update pina_mb_hashpower set pool3_hashp = round((pool3_hashp +  $p3_repurchase), 4) where mb_id='$id'";
			sql_query($up_repurchase);
			$cnt = $cnt+1;
			save_mining_slog(3, $p3_profit, $p3_repurchase, $id, $to_date, $cnt,$bitrate,$etherate);
		}
		if($p4_profit>=0){
			echo $up_repurchase = "update pina_mb_hashpower set pool4_hashp =round((pool4_hashp +  $p4_repurchase), 4) where mb_id='$id'";
			sql_query($up_repurchase);
			$cnt = $cnt+1;
		...
		if($p5_profit>=0){
			...
			$up_repurchase = "update pina_mb_hashpower set pool5_hashp = round((pool5_hashp +  $p5_repurchase), 4) where mb_id='$id'";
			sql_query($up_repurchase);
		}
		if($gpu_profit>=0){
			$cnt = $cnt+1;
