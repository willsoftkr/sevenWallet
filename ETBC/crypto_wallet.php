...
<head>
	...
	<script>
		...
		$(function() {
			...
			$('#verification_email').on('click', function() {//메일 인증 발사
				var mb_id =  '<? echo $member[mb_id]?>';
				var mb_email =  '<? echo $member[mb_email]?>';
				$.ajax({
					type: "POST",
					url: "manage_profile_verification_email.php",
					cache: false,
					async: false,
					dataType: "json",
					data:  {
			...
			$('#btn_btc').on('click', function() {
				...
				$.ajax({
					...
					success: function(data) {
						...
						location.replace("./crypto_wallet.php");
					}
				});
			});
			
			//bit transfer to another member
			$('#send_btc').on('click', function() {
				if(!mb_block){
					$.ajax({
						type: "POST",
			...
			$('#btc_withd_btn').on('click', function() {
				if(!mb_block){
					$.ajax({
						type: "POST",
						url: "coin_trans_proc.php",
			...
			$('#withd_btc').on('keyup',function(e){
				var btc = '<?=$cost_btc_js['btc_cost']?>'; // 1비트코인의 환전 금액
		...
		var priceChartsConfig = {
			...
			data: {
				...
				datasets: [{
					...
					data: JSON.parse('<? echo json_encode($btc_arr);?>'),
					fill: false,
				}]
			},
			options: {
	...
	<div class="main-container">
		<div id="body-wrapper"class="big-container-wrapper">
			<div class="crypto-wallets-container">
				<h2 class="gray" data-i18n="wallet.title" >BTC Wallet</h2>
					...
					<div class="wallet-selector-container">
						<div class="bitcoin-wallet active">
							...
							<div class="bitcoin-wallet-holding-amount">
								<span class="blue">$ <?=round($coin_dollor['btc_cost']*$btc_wallet,2)?></span><br>
								<span class="gray"><?=$btc_wallet?> BTC</span>
							</div>
							<div class="bitcoin-wallet-pct-change">
								<span class="green"></span>
							</div>
							<div class="input-group mb-3 wallet-address-container">
								<input type="text" class="form-control" placeholder="BTC Wallet Address" aria-label="BTC Wallet Address" aria-describedby="basic-addon2" id="btc_my_wallet" value="<?echo $member['btc_my_wallet'];?>" data-i18n="[placeholder]wallet.addr" >
								<div class="input-group-append">
									<button class="btn btn-outline-primary save-button" type="button" id="btn_btc" data-i18n="wallet.save" >Save</button>
				...
				<div class="wallets-right-container">
					<div class="transaction-panels">
						...
						<div id="transfer" class="panel active">
							...
							<form class="transfer-form">
								...
								<div class="otp-auth-code-container">
									<i id="verification_email" class="fas fa-paper-plane"></i>
									<input type="text" id="token_str" data-i18n="[placeholder]wallet.mailCode" name="otp_code" placeholder="Mail Authorization Code" required >
								</div>
								<div class="otp-auth-code-container">
								...
								<div class="send-button-container">
									...
									<button type="button" class="btn btn-primary form-send-button" data-i18n="wallet.sendBit" id="send_btc">Send Bitcoin</button>
						...
						<input type="hidden" value="<? echo $member[btc_my_wallet];?>" id="withdrawal-address"/>
						<div id="withdraw" class="panel">
							<form class="transfer-form">
								...
								<!--data-toggle="modal" data-target="#withdrawBitcoin"-->
									<button type="button" class="btn btn-primary form-send-button" id="btc_withd_btn" data-i18n="wallet.withdrawBitcoin">Withdraw Bitcoin</button>
									
									<br>
									<span data-i18n="wallet.withdrawlFee" >Withdrawal fee</span>: 0.002 BTC<br>
									<span class="gray">(<span data-i18n="wallet.minWithdrawal" >Minimum Withdrawal</span> 0.02 BTC)</span>
								</div>
							</form>
						</div>
...
<?
	...
	$sql .= " WHERE M.mb_id = '".$member['mb_id']."' and type in(1, 2)";
	$row = sql_fetch($sql);
	$total_count = $row['cnt'];
	...
	$list = sql_query($sql);
//type / {amount} BTC to {username} <-- transfer from A to B
// withdrawal of {amount} BTC
?>
				...
				<table class="table table-hover table-responsive">
					...
					<thead>
						<tr>
							...
							<th >BTC</th> <!--data-i18n="wallet.amount"-->
					...
					<tbody>
						<?for ($i=0; $row=sql_fetch_array($list); $i++) {?>
							<tr>
								...
								<td>
									<?if($row['type']==1){echo "Transferred ".$row[amt]." BTC to ".$row[recipient];}
									else if($row['type']==2){echo "withdrawal of ".$row[amt]." BTC";}?>
								</td>
								<td>
