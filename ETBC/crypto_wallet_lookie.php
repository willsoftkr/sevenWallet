		...
		$(function() {
			...
			$('#btn_lkc').on('click', function() {
				$.ajax({
					...
					data:  {
						coin : 'lkc',
						lkc_my_wallet : $('#lkc_my_wallet').val()
					},
					success: function(data) {
						$('#bitcoinAddressModalCenter').modal('show');
						location.replace("./crypto_wallet_lookie.php");
					}
				});
			});
			
			//lkc transfer to another member
			$('#send_lkc').on('click', function() {
				if(!mb_block){
					$.ajax({
						type: "POST",
			...
			$('#lkc_withd_btn').on('click', function() {
				if(!mb_block){
					$.ajax({
						type: "POST",
						url: "coin_trans_proc.php",
			...
			$('#withd_btc').on('keyup',function(e){
				var lkc = '<?=$cost_btc_js['lookie_cost']?>'; // 1LKC의 환전 금액
				$('#withd_usd').val(Number($('#withd_btc').val()) * Number(lkc));
			});

			$('#trans_lkc').on('keyup',function(e){

				var lkc = '<?=$cost_btc_js['lookie_cost']?>'; // 1LKC의 환전 금액

				$('#trans_usd').val(Number($('#trans_lkc').val()) * Number(lkc));
			});
		...
		var priceChartsConfig = {
			...
			data: {
				...
				datasets: [{
					...
					data: JSON.parse('<? echo json_encode($lkc_arr);?>'),
					fill: false,
				}]
			},
			options: {
	...
	<div class="main-container">
		<div id="body-wrapper"class="big-container-wrapper">
			<div class="crypto-wallets-container">
				<h2 class="gray" data-i18n="walletLKC.title" >LKC Wallet</h2>
					...
					<div class="wallet-selector-container">
						<div class="bitcoin-wallet active">
							<div class="bitcoin-wallet-logo">
								<img src="./images/lkc_logo.png">
							</div>
							<div class="bitcoin-wallet-current-price">
								<span class="blue">LKC</span><br>
								<span class="gray">$ <?=$coin_dollor['lookie_cost']?></span>
							</div>
							<div class="bitcoin-wallet-holding-amount">
								<span class="blue">$ <?=round($coin_dollor['lookie_cost']*$lkc_num,2)?></span><br>
								<span class="gray"><?=$lkc_num?> LKC</span>
							</div>
							<div class="bitcoin-wallet-pct-change">
								<span class="green"></span>
							</div>
							<div class="input-group mb-3 wallet-address-container"><!--출금 주소 변경-->
								<input type="text" class="form-control" placeholder="LKC Wallet Address" aria-label="LKC Wallet Address" aria-describedby="basic-addon2" id="lkc_my_wallet" value="<?echo $lkc_addr;?>" data-i18n="[placeholder]walletLKC.addr" >
								<div class="input-group-append">
									<button class="btn btn-outline-primary save-button" type="button" id="btn_lkc" data-i18n="wallet.save" >Save</button>
									
								</div>
							</div>
						</div>
						<div class="price-chart-container-wallet shadow" >
							<h5  style="text-align:center" data-i18n="walletLKC.chart">
								 7 Days Price Charts
							</h5>
				...
				<div class="wallets-right-container">
					<div class="transaction-panels">
							...
							<form class="transfer-form">
								...
								<div class="btc-usd-exchange-container">
									<div class="crypto-exchange-price">
										LKC : <input type="text" placeholder="0" id="trans_lkc" name="trans_lkc">
									</div>
								...
								<div class="send-button-container">
									...
									<button type="button" class="btn btn-primary form-send-button" data-i18n="walletLKC.send" id="send_lkc">Send Bitcoin</button>
						...
						<input type="hidden" value="<? echo $lkc_addr;?>" id="withdrawal-address"/>
						<div id="withdraw" class="panel">
							<form class="transfer-form">
								...
								<div class="btc-usd-exchange-container">
									<div class="crypto-exchange-price">
										LKC : <input type="text" placeholder="0" id="withd_btc" name="withd_btc">
									</div>
									<i class="fas fa-exchange-alt"></i>
									<div class="crypto-exchange-price">
								...
								<!--data-toggle="modal" data-target="#withdrawBitcoin"-->
									<button type="button" class="btn btn-primary form-send-button" id="lkc_withd_btn" data-i18n="walletLKC.withdrawRockWood">Withdraw Bitcoin</button>
									
									<br>
									<span data-i18n="wallet.withdrawlFee" >Withdrawal fee</span>: 1 LKC<br>
									<span class="gray">(<span data-i18n="wallet.minWithdrawal" >Minimum Withdrawal</span> 10 LKC)</span>
								</div>
							</form>
						</div>
...
<?
	...
	$sql .= " WHERE M.mb_id = '".$member['mb_id']."' and type in(7, 8)";
	$row = sql_fetch($sql);
	$total_count = $row['cnt'];
	...
	$list = sql_query($sql);
//type / {amount} LKC to {username} <-- transfer from A to B
// withdrawal of {amount} LKC
?>
				...
				<table class="table table-hover table-responsive">
					...
					<thead>
						<tr>
							...
							<th >LKC</th> <!--data-i18n="wallet.amount"-->
					...
					<tbody>
						<?for ($i=0; $row=sql_fetch_array($list); $i++) {?>
							<tr>
								...
								<td>
									<?if($row['type']==7){echo "Transferred ".$row[amt]." LKC to ".$row[recipient];}
									else if($row['type']==8){echo "withdrawal of ".$row[amt]." LKC";}?>
								</td>
								<td>
