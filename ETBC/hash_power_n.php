...
$mining_pw_sql = "SELECT * FROM pina_mb_hashpower where mb_id='rose'";

$pool_sql = "SELECT c.*, substring(ct_time,1,10) as p_date FROM g5_shop_cart c LEFT JOIN g5_shop_order o ON c.od_id = o.od_id 
                 WHERE c.mb_id = 'rose' AND o.od_status IN ( '입금', '강제입금') ORDER BY c.it_name ASC ";
$hash_arr = array();
...
$repurchase = array('1527096045' => 'p1_repurchase', '1527096041' => 'p2_repurchase', '1527096037' => 'p3_repurchase', '1527096030' => 'p4_repurchase', '1526013457' => 'package 5',  'VVIP112500'=>'VVIP 1','VVIP225000'=>'VVIP 2' , 'VVIP337500'=>'VVIP 3', 'VVIP450000'=>'VVIP 4', '1515148167' => 'package GPU') ;
...
sum(re_purchase_poolg) FROM `pina_mining_profit` WHERE 1 and mb_id='rose' ";
sql_fetch($set_init);

$get_now = "select * from pina_mb_hashpower where mb_id = 'rose' ";
...
re_purchase_poolg FROM `pina_mining_profit` WHERE 1 and mb_id='rose' and profit_date >='2018-12-05' order by profit_date desc";
...
<head>
	...
	<link rel="stylesheet" href="css/hash_power/style.css">
	<meta charset="utf-8">
						...
						<?while($row_p = sql_fetch_array($pool_list)) {
							...
							<li rel="<?echo $package_name[$row_p[it_id]]?>" class="active">
								...
								<div class="adjust-repurchase-rate">
									<select class="custom-select" id="item1">
						...
						<table class="table table-hover">
						  ...
						  while($row= sql_fetch_array($rest_list)){
						    ...
						    <tr>
						      ...
						      <td><? echo $hash-  $row[re_purchase_pool1] ; $hash = $hash -  $row[re_purchase_pool1] ?> GH/s</td>
						      <td><?=$row[re_purchase_pool1]?> GH/s</td>
						      <td><span><?echo $hash?> GH/s</span></td>
						    </tr>
								<?}?>
						  </tbody>
						</table>
							...
							<tbody>
							  ...
							  while($row= sql_fetch_array($rest_list)){
								...
								<tr>
								  ...
								  <td><? echo $hash2-  $row[re_purchase_pool2]; $hash2 = $hash2 -  $row[re_purchase_pool2] ?> GH/s</td>
								  <td><?=$row[re_purchase_pool2]?> GH/s</td>
								  <td><span><?echo $hash2 ?> GH/s</span></td>
								</tr>
									<?}?>
							</tbody>
						</table>
						...
						<table class="table table-hover">
						  ...
						  <tbody>
						    ...
						    <tr>
						      ...
						      <td><? echo $hash4-  $row[re_purchase_pool4]; $hash4 = $hash4 -  $row[re_purchase_pool4] ?> GH/s</td>
						      <td><?=$row[re_purchase_pool3]?> GH/s</td>
						      <td><span><?echo $hash4 ?> GH/s</span></td>
						    </tr>
							<?}?>
						  </tbody>
						</table>
						...
						<table class="table table-hover">
						  ...
						  <tbody>
						    ...
						    <tr>
						      ...
						      <td><? echo $hash5 -  $row[re_purchase_pool5] ; $hash5 = $hash5 ?> GH/s</td>
						      <td><?=$row[re_purchase_pool5]?> GH/s</td>
						      <td><span><?echo $hash5 ?> GH/s</span></td>
						    </tr>
							<?}?>
						  </tbody>
						</table>
						...
						<table class="table table-hover">
						  ...
						  <tbody>
						    ...
						    <tr>
						      ...
						      <td><? echo $hashg; $hashg = $hashg -  $row[re_purchase_poolg] ?> GH/s</td>
						      <td><?=$row[re_purchase_poolg]?> GH/s</td>
						      <td><span><?echo $hashg+$row[re_purchase_poolg]*2 ?> GH/s</span></td>
						    </tr>
							<?}?>
						  </tbody>
						</table>
