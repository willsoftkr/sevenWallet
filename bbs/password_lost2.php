...
if (!$email)
	...
	alert_modal('It is not a valid email address.');
...
$mb = sql_fetch($sql);
if (!$mb['mb_id'])
	alert_modal("No one has the email you entered.<br>{$email}",'/bbs/login.php');
...
mailer($config['cf_admin_email_name'], 'noreply@pinnaclemining.net', $email , $subject, $content, 1);
...
alert_modal("Password and username verification email has been sent.<br><br>Check your email. If you do not receive it check your spam folder first and then open a support ticket for more help.<br><br>{$email}",'/bbs/login.php');
